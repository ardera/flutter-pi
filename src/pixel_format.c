...
#define PIXFMT_MAPPING(                                                                                                  \
    ...
    _drm_format                                                                                                          \
      ...
      GBM_FORMAT_FIELD_INITIALIZER(_gbm_format) DRM_FORMAT_FIELD_INITIALIZER(_drm_format)                                \
},
// clang-format on

const struct pixfmt_info pixfmt_infos[] = { PIXFMT_LIST(PIXFMT_MAPPING) };

/// hack so we can use COMPILE_ASSERT.
enum { n_pixfmt_infos_constexpr = sizeof(pixfmt_infos) / sizeof(*pixfmt_infos) };
