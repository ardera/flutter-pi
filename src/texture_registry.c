...
int texreg_register_texture(
    ...
    *details = (struct texture_details) {
    ...
    engine_result = FlutterEngineRegisterExternalTexture(engine, tex_id);
...
int texreg_mark_texture_frame_available(int64_t texture_id) {
    ...
    engine_result = FlutterEngineMarkExternalTextureFrameAvailable(engine, texture_id);
    if (engine_result != kSuccess) {
        return EINVAL;
    }
...
int texreg_unregister_texture(int64_t texture_id) {
    ...
    engine_result = FlutterEngineUnregisterExternalTexture(engine, texture_id);
    if (engine_result != kSuccess) {
        return EINVAL;
    }
}