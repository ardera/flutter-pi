...
static bool on_make_current(void *userdata)
{
	...
	eglMakeCurrent(flutterpi.egl.display, flutterpi.egl.surface,
				   flutterpi.egl.surface, flutterpi.egl.flutter_render_context);
	if (egl_error = eglGetError(), egl_error != EGL_SUCCESS)
	{
		fprintf(stderr,
				"[flutter-pi] Could not make the flutter rendering EGL context "
				"current. eglMakeCurrent: 0x%08X\n",
				egl_error);
		return false;
	}
...
static bool on_clear_current(void *userdata)
{
	...
	{
		fprintf(stderr,
				"[flutter-pi] Could not clear the flutter EGL context. "
				"eglMakeCurrent: 0x%08X\n",
				egl_error);
		return false;
	}
...
/// (Won't be called since we're supplying a compositor,
/// still needs to be present)
static uint32_t fbo_callback(void *userdata) { return 0; }
...
static bool on_make_resource_current(void *userdata)
{
	...
	{
		fprintf(stderr,
				"[flutter-pi] Could not make the flutter resource uploading EGL "
				"context current. eglMakeCurrent: 0x%08X\n",
				egl_error);
		return false;
	}
...
static const GLubyte *hacked_glGetString(GLenum name)
{
	...
	if (extensions == NULL)
	{
		...
		{
			fprintf(stderr,
					"Could not allocate memory for modified GL_EXTENSIONS string\n");
			return NULL;
		}
     ...
     */
		...
		// cut_word_from_string(extensions,
...
static void *proc_resolver(void *userdata, const char *name)
{
	...
	/*
   * The mesa V3D driver reports some OpenGL ES extensions as supported and
   * working even though they aren't. hacked_glGetString is a workaround for
   * this, which will cut out the non-working extensions from the list of
   ...
   */
	...
	if ((is_VC4 == -1) &&
		(is_VC4 = strcmp(flutterpi.egl.renderer, "VC4 V3D 2.1") == 0))
		printf("detected VideoCore IV as underlying graphics chip, and VC4 as the "
			   "driver.\n"
			   "Reporting modified GL_EXTENSIONS string that doesn't contain "
			   "non-working extensions.\n");

	// if we do, and the symbol to resolve is glGetString, we return our
	// hacked_glGetString.
	...
	fprintf(stderr,
			"[flutter-pi] proc_resolver: Could not resolve symbol \"%s\"\n",
			name);

	return NULL;
}

static void on_platform_message(const FlutterPlatformMessage *message,
								void *userdata)
{
	int ok;
	...
	{
		fprintf(stderr,
				"[flutter-pi] Error handling platform message. "
				"plugin_registry_on_platform_message: %s\n",
				strerror(ok));
	}
}
...
/// the frame request.
static int on_execute_frame_request(void *userdata)
{
	...
	if (ok == 0)
	{
		if (peek->state == kFramePending)
		{
			...
			{
				ns = 0;
				ok = drmCrtcGetSequence(
					flutterpi.drm.drmdev->fd,
					flutterpi.drm.drmdev->selected_crtc->crtc->crtc_id, NULL, &ns);
				if (ok < 0)
				{
					perror("[flutter-pi] Couldn't get last vblank timestamp. "
						   "drmCrtcGetSequence");
			...
			result = flutterpi.flutter.libflutter_engine.FlutterEngineOnVsync(
				flutterpi.flutter.engine, peek->baton, ns,
				ns + (1000000000 / flutterpi.display.refresh_rate));
			if (result != kSuccess)
			{
				fprintf(stderr,
						"[flutter-pi] Could not reply to frame request. "
						"FlutterEngineOnVsync: %s\n",
	...
	{
		fprintf(stderr,
				"[flutter-pi] Could not get peek of frame queue. "
				"cqueue_peek_locked: %s\n",
				strerror(ok));
		cqueue_unlock(&flutterpi.frame_queue);
		return ok;
	}
...
static void on_frame_request(void *userdata, intptr_t baton)
{
	...
	if ((ok == 0) || (ok == EAGAIN))
	{
		...
		ok = cqueue_try_enqueue_locked(
			&flutterpi.frame_queue,
			&(struct frame){.state = kFramePending, .baton = baton});
		if (ok != 0)
		{
			fprintf(stderr,
					"[flutter-pi] Could not enqueue frame request. "
					"cqueue_try_enqueue_locked: %s\n",
					strerror(ok));
			cqueue_unlock(&flutterpi.frame_queue);
			return;
		}
		...
		if (reply_instantly)
		{
			flutterpi_post_platform_task(on_execute_frame_request, NULL);
		}
	}
	else if (ok != 0)
	{
...
int flutterpi_post_platform_task(int (*callback)(void *userdata),
								 void *userdata)
{
	struct platform_task *task;
	int ok;
	...
	ok = sd_event_add_defer(flutterpi.event_loop, NULL, on_execute_platform_task,
							task);
	if (ok < 0)
	{
		fprintf(stderr,
				"[flutter-pi] Error posting platform task to main loop. "
	...
	if (pthread_self() != flutterpi.event_loop_thread)
	{
		ok = write(flutterpi.wakeup_event_loop_fd,
				   (uint8_t[8]){0, 0, 0, 0, 0, 0, 0, 1}, 8);
		if (ok < 0)
		{
...
static int on_execute_platform_task_with_time(sd_event_source *s, uint64_t usec,
											  void *userdata)
{
	struct platform_task *task;
	int ok;
...
int flutterpi_post_platform_task_with_time(int (*callback)(void *userdata),
										   void *userdata,
										   uint64_t target_time_usec)
{
	...
	ok = sd_event_add_time(flutterpi.event_loop, NULL, CLOCK_MONOTONIC,
						   target_time_usec, 1,
						   on_execute_platform_task_with_time, task);
	if (ok < 0)
	{
		fprintf(stderr,
				"[flutter-pi] Error posting platform task to main loop. "
...
int flutterpi_sd_event_add_io(sd_event_source **source_out, int fd,
							  ...
							  void *userdata)
{
	...
	ok = sd_event_add_io(flutterpi.event_loop, source_out, fd, events, callback,
						 userdata);
	if (ok < 0)
	{
		fprintf(stderr,
				"[flutter-pi] Could not add IO callback to event loop. "
				"sd_event_add_io: %s\n",
				strerror(-ok));
		return -ok;
	}
...
static int on_execute_flutter_task(void *userdata)
{
	...
	result = flutterpi.flutter.libflutter_engine.FlutterEngineRunTask(
		flutterpi.flutter.engine, task);
	if (result != kSuccess)
	{
		fprintf(
			stderr,
			"[flutter-pi] Error running platform task. FlutterEngineRunTask: %d\n",
			result);
		free(task);
		return EINVAL;
	}
...
static void on_post_flutter_task(FlutterTask task, uint64_t target_time,
								 void *userdata)
{
	sd_event_source *source;
	...
	*dup_task = task;
	...
	ok = flutterpi_post_platform_task_with_time(on_execute_flutter_task, dup_task,
												target_time / 1000);
	if (ok != 0)
	{
		free(dup_task);
...
static int on_send_platform_message(void *userdata)
{
	...
	if (msg->is_response)
	{
		result = flutterpi.flutter.libflutter_engine
					 .FlutterEngineSendPlatformMessageResponse(
						 flutterpi.flutter.engine, msg->target_handle, msg->message,
						 msg->message_size);
	}
	else
	{
		result =
			flutterpi.flutter.libflutter_engine.FlutterEngineSendPlatformMessage(
	...
	{
		fprintf(stderr,
				"[flutter-pi] Error sending platform message. "
				"FlutterEngineSendPlatformMessage: %s\n",
				FLUTTER_RESULT_TO_STRING(result));
	}
...
int flutterpi_send_platform_message(
	const char *channel, const uint8_t *restrict message, size_t message_size,
	FlutterPlatformMessageResponseHandle *responsehandle)
{
	...
	if (runs_platform_tasks_on_current_thread(NULL))
	{
		result =
			flutterpi.flutter.libflutter_engine.FlutterEngineSendPlatformMessage(
				flutterpi.flutter.engine,
				&(const FlutterPlatformMessage){.struct_size =
													sizeof(FlutterPlatformMessage),
												.channel = channel,
												.message = message,
	...
	const uint8_t *restrict message, size_t message_size)
{
	...
	if (runs_platform_tasks_on_current_thread(NULL))
	{
		result = flutterpi.flutter.libflutter_engine
					 .FlutterEngineSendPlatformMessageResponse(
						 flutterpi.flutter.engine, handle, message, message_size);
		if (result != kSuccess)
		{
			fprintf(stderr,
					"[flutter-pi] Error sending platform message response. "
					"FlutterEngineSendPlatformMessageResponse: %s\n",
					FLUTTER_RESULT_TO_STRING(result));
			return EIO;
		}
	}
...
static int run_main_loop(void)
{
	...
	{
		fprintf(stderr,
				"[flutter-pi] Could not get fd for main event loop. "
				"sd_event_get_fd: %s\n",
				strerror(-ok));
		pthread_mutex_unlock(&flutterpi.event_loop_mutex);
		return -ok;
...

	{
		...
		do
		{
			...
			case SD_EVENT_INITIAL:
				...
				{
					fprintf(stderr,
							"[flutter-pi] Could not prepare event loop. "
							"sd_event_prepare: %s\n",
							strerror(-ok));
					return -ok;
				}
			...
			case SD_EVENT_ARMED:
				...
				{
					fprintf(stderr,
							"[flutter-pi] Could not check for event loop events. "
							"sd_event_wait: %s\n",
							strerror(-ok));
					return -ok;
				}
			...
			case SD_EVENT_PENDING:
				...
				{
					fprintf(stderr,
							"[flutter-pi] Could not dispatch event loop events. "
							"sd_event_dispatch: %s\n",
							strerror(-ok));
					return -ok;
				}
			...
			default:
				fprintf(stderr,
						"[flutter-pi] Unhandled event loop state: %d. Aborting\n",
						state);
				abort();
			}
...
static int on_wakeup_main_loop(sd_event_source *s, int fd, uint32_t revents,
							   void *userdata)
{
	uint8_t buffer[8];
	int ok;
...
static int init_main_loop(void)
{
	...
	if (wakeup_fd < 0)
	{
		perror("[flutter-pi] Could not create fd for waking up the main loop. "
			   "eventfd");
		return errno;
	}
	...
	{
		fprintf(stderr,
				"[flutter-pi] Could not create main event loop. sd_event_new: %s\n",
				strerror(-ok));
		return -ok;
	}

	ok = sd_event_add_io(flutterpi.event_loop, NULL, wakeup_fd, EPOLLIN,
						 on_wakeup_main_loop, NULL);
	if (ok < 0)
	{
		fprintf(stderr,
				"[flutter-pi] Error adding wakeup callback to main loop. "
				"sd_event_add_io: %s\n",
				strerror(-ok));
		sd_event_unrefp(&flutterpi.event_loop);
		close(wakeup_fd);
...
void on_pageflip_event(int fd, unsigned int frame, unsigned int sec,
					   unsigned int usec, void *userdata)
{
	...
	flutterpi.flutter.libflutter_engine.FlutterEngineTraceEventInstant(
		"pageflip");
	...
	if (ok != 0)
	{
		fprintf(
			stderr,
			"[flutter-pi] Could not dequeue completed frame from frame queue: %s\n",
			strerror(ok));
	...
	else if (ok != 0)
	{
		fprintf(
			stderr,
			"[flutter-pi] Could not get frame queue peek. cqueue_peek_locked: %s\n",
			strerror(ok));
		goto fail_unlock_frame_queue;
	}
	else
	{
		...
		{
			fprintf(stderr,
					"[flutter-pi] frame queue in inconsistent state. aborting\n");
			abort();
		}
	}
	...
	{
		fprintf(stderr,
				"[flutter-pi] Error notifying compositor about page flip. "
				"compositor_on_page_flip: %s\n",
				strerror(ok));
	}
...
static int on_drm_fd_ready(sd_event_source *s, int fd, uint32_t revents,
						   void *userdata)
{
	int ok;
...
int flutterpi_fill_view_properties(bool has_orientation,
								   enum device_orientation orientation,
								   bool has_rotation, int rotation)
{
	enum device_orientation default_orientation =
		flutterpi.display.width >= flutterpi.display.height ? kLandscapeLeft
															: kPortraitUp;
	...
	if (flutterpi.view.has_orientation)
	{
		...
		{
			flutterpi.view.rotation = ANGLE_BETWEEN_ORIENTATIONS(
	...
	else if (flutterpi.view.has_rotation)
	{
		...
		{
			if (ANGLE_BETWEEN_ORIENTATIONS(default_orientation, i) ==
	...
	{
		flutterpi.view.rotation +=
			ANGLE_BETWEEN_ORIENTATIONS(flutterpi.view.orientation, orientation);
	...
	if ((flutterpi.view.rotation <= 45) ||
		((flutterpi.view.rotation >= 135) && (flutterpi.view.rotation <= 225)) ||
	...
	else if (flutterpi.view.rotation == 180)
	{
		...
		flutterpi.view.display_to_view_transform =
			FLUTTER_ROTZ_TRANSFORMATION(-180);
	...
	if (flutterpi.user_input != NULL)
	{
		...
		user_input_set_transform(flutterpi.user_input,
								 &flutterpi.view.display_to_view_transform,
								 &flutterpi.view.view_to_display_transform,
...
static const FlutterLocale *
on_compute_platform_resolved_locales(const FlutterLocale **locales,
									 size_t n_locales)
{
	return locales_on_compute_platform_resolved_locale(flutterpi.locales, locales,
													   n_locales);
}
...
static int init_display(void)
{
   ...
   **********************/
	...
	for (int i = 0; i < num_devices; i++)
	{
		...
		ok = drmdev_new_from_path(&flutterpi.drm.drmdev,
								  device->nodes[DRM_NODE_PRIMARY]);
		if (ok != 0)
		{
			fprintf(stderr,
					"[flutter-pi] Could not create drmdev from device at \"%s\". "
					"Continuing.\n",
					device->nodes[DRM_NODE_PRIMARY]);
			continue;
		}
	...
	if (flutterpi.drm.drmdev == NULL)
	{
		fprintf(
			...
			"If you're not using a Raspberry Pi, please make sure there's KMS "
			"support for your graphics chip.\n");
		return ENOENT;
	}
	...
	for_each_connector_in_drmdev(flutterpi.drm.drmdev, connector)
	{
		if (connector->connector->connection == DRM_MODE_CONNECTED)
		{
			...
			if ((flutterpi.display.width_mm == 0) ||
	...
	// Find the preferred mode (GPU drivers _should_ always supply a preferred
	// mode, but of course, they don't) Alternatively, find the mode with the
	// highest width*height. If there are multiple modes with the same w*h, prefer
	// higher refresh rates. After that, prefer progressive scanout modes.
	mode = NULL;
	...
	{
		fprintf(stderr,
				"[flutter-pi] WARNING: display didn't provide valid physical "
				"dimensions.\n"
				"             The device-pixel ratio will default to 1.0, which "
				"may not be the fitting device-pixel ratio for your display.\n");
		flutterpi.display.pixel_ratio = 1.0;
	}
	else
	{
		flutterpi.display.pixel_ratio =
			(10.0 * flutterpi.display.width) / (flutterpi.display.width_mm * 38.0);

		int horizontal_dpi =
			(int)(flutterpi.display.width / (flutterpi.display.width_mm / 25.4));
		int vertical_dpi =
			(int)(flutterpi.display.height / (flutterpi.display.height_mm / 25.4));

		if (horizontal_dpi != vertical_dpi)
		{
			// See https://github.com/flutter/flutter/issues/71865 for current status
			// of this issue.
			fprintf(stderr, "[flutter-pi] WARNING: display has non-square pixels. "
							"Non-square-pixels are not supported by flutter.\n");
		}
	}
	...
	if (encoder == NULL)
	{
		for (int i = 0; i < connector->connector->count_encoders;
			 i++, encoder = NULL)
		{
			for_each_encoder_in_drmdev(flutterpi.drm.drmdev, encoder)
			{
	...
	ok =
		drmdev_configure(flutterpi.drm.drmdev, connector->connector->connector_id,
						 encoder->encoder->encoder_id, crtc->crtc->crtc_id, mode);
	if (ok != 0)
		return ok;
	...
	{
		uint64_t ns = 0;
		ok = drmCrtcGetSequence(flutterpi.drm.drmdev->fd,
								flutterpi.drm.drmdev->selected_crtc->crtc->crtc_id,
								NULL, &ns);
		int _errno = errno;
		...
		else
		{
			...
			{
				fprintf(stderr,
						"WARNING: Error getting last vblank timestamp. "
						"drmCrtcGetSequence: %s\n",
						strerror(_errno));
			}
			else
			{
				fprintf(stderr, "WARNING: Kernel didn't return a valid vblank "
								"timestamp. (timestamp == 0)\n");
			}
			fprintf(stderr,
					"         VSync will be disabled.\n"
					"         See https://github.com/ardera/flutter-pi/issues/38 for "
					"more info.\n");
		}
	}
	...
	ok = sd_event_add_io(flutterpi.event_loop,
						 &flutterpi.drm.drm_pageflip_event_source,
						 flutterpi.drm.drmdev->fd, EPOLLIN | EPOLLHUP | EPOLLPRI,
						 on_drm_fd_ready, NULL);
	if (ok < 0)
	{
		fprintf(stderr,
				"[flutter-pi] Could not add DRM pageflip event listener. "
				"sd_event_add_io: %s\n",
				strerror(-ok));
		return -ok;
	}
	...
	printf("===================================\n"
		   ...
		   "===================================\n",
		   flutterpi.display.width, flutterpi.display.height,
		   flutterpi.display.refresh_rate, flutterpi.display.width_mm,
		   flutterpi.display.height_mm, flutterpi.display.pixel_ratio);
   ...
   **********************/
	...
	flutterpi.gbm.surface = gbm_surface_create_with_modifiers(
		flutterpi.gbm.device, flutterpi.display.width, flutterpi.display.height,
		flutterpi.gbm.format, &flutterpi.gbm.modifier, 1);
	if (flutterpi.gbm.surface == NULL)
	{
		perror("[flutter-pi] Could not create GBM Surface. "
   ...
   **********************/
	...
	{
		fprintf(stderr,
				"[flutter-pi] Could not load EGL / GL ES procedure addresses! "
				"error: %s\n",
				strerror(ok));
		return ok;
	}
	...
	flutterpi.egl.display = flutterpi.egl.getPlatformDisplay(
		EGL_PLATFORM_GBM_KHR, flutterpi.gbm.device, NULL);
	if ((egl_error = eglGetError()) != EGL_SUCCESS)
	{
		fprintf(stderr,
				"[flutter-pi] Could not get EGL display! eglGetPlatformDisplay: "
				"0x%08X\n",
				egl_error);
		return EIO;
	}
	...
	{
		fprintf(stderr,
				"[flutter-pi] Could not get EGL display! eglGetDisplay: 0x%08X\n",
				egl_error);
		return EIO;
	}
#endif
	...
	{
		fprintf(stderr,
				"[flutter-pi] Failed to initialize EGL! eglInitialize: 0x%08X\n",
				egl_error);
		return EIO;
	}
	...
	{
		fprintf(stderr,
				"[flutter-pi] Failed to bind OpenGL ES API! eglBindAPI: 0x%08X\n",
				egl_error);
		return EIO;
	}
	...
	{
		fprintf(stderr,
				"[flutter-pi] Could not get the number of EGL framebuffer "
				"configurations. eglGetConfigs: 0x%08X\n",
				egl_error);
		return EIO;
	}
	...
	eglChooseConfig(flutterpi.egl.display, config_attribs, configs, count,
					&matched);
	if ((egl_error = eglGetError()) != EGL_SUCCESS)
	{
		fprintf(stderr,
				"[flutter-pi] Could not query EGL framebuffer configurations with "
				"fitting attributes. eglChooseConfig: 0x%08X\n",
				egl_error);
		return EIO;
	}
	...
	{
		fprintf(stderr,
				"[flutter-pi] No fitting EGL framebuffer configuration found.\n");
		return EIO;
	}
	...
	for (int i = 0; i < count; i++)
	{
		...
		eglGetConfigAttrib(flutterpi.egl.display, configs[i], EGL_NATIVE_VISUAL_ID,
						   &native_visual_id);
		if ((egl_error = eglGetError()) != EGL_SUCCESS)
		{
			fprintf(stderr,
					"[flutter-pi] Could not query native visual ID of EGL config. "
					"eglGetConfigAttrib: 0x%08X\n",
					egl_error);
			continue;
		}
	...
	{
		fprintf(stderr, "[flutter-pi] Could not find EGL framebuffer configuration "
						"with appropriate attributes & native visual ID.\n");
		return EIO;
	}
   ...
   ****************************/
	flutterpi.egl.root_context =
		eglCreateContext(flutterpi.egl.display, flutterpi.egl.config,
						 EGL_NO_CONTEXT, context_attribs);
	if ((egl_error = eglGetError()) != EGL_SUCCESS)
	{
		fprintf(stderr,
				"[flutter-pi] Could not create OpenGL ES root context. "
				"eglCreateContext: 0x%08X\n",
				egl_error);
		return EIO;
	}
	...
	{
		fprintf(stderr,
				"[flutter-pi] Could not create OpenGL ES context for flutter "
				"rendering. eglCreateContext: 0x%08X\n",
				egl_error);
		return EIO;
	}
	...
	if ((egl_error = eglGetError()) != EGL_SUCCESS)
	{
		fprintf(stderr,
				"[flutter-pi] Could not create OpenGL ES context for flutter "
				"resource uploads. eglCreateContext: 0x%08X\n",
				egl_error);
		return EIO;
	}

	flutterpi.egl.compositor_context =
		eglCreateContext(flutterpi.egl.display, flutterpi.egl.config,
						 flutterpi.egl.root_context, context_attribs);
	if ((egl_error = eglGetError()) != EGL_SUCCESS)
	{
		fprintf(stderr,
				"[flutter-pi] Could not create OpenGL ES context for compositor. "
				"eglCreateContext: 0x%08X\n",
				egl_error);
		return EIO;
	}
	...
	flutterpi.egl.surface =
		eglCreateWindowSurface(flutterpi.egl.display, flutterpi.egl.config,
							   (EGLNativeWindowType)flutterpi.gbm.surface, NULL);
	if ((egl_error = eglGetError()) != EGL_SUCCESS)
	{
		fprintf(stderr,
				"[flutter-pi] Could not create EGL window surface. "
				"eglCreateWindowSurface: 0x%08X\n",
				egl_error);
		return EIO;
	}

	eglMakeCurrent(flutterpi.egl.display, flutterpi.egl.surface,
				   flutterpi.egl.surface, flutterpi.egl.root_context);
	if ((egl_error = eglGetError()) != EGL_SUCCESS)
	{
		fprintf(stderr,
				"[flutter-pi] Could not make OpenGL ES root context current to get "
				"OpenGL information. eglMakeCurrent: 0x%08X\n",
				egl_error);
		return EIO;
	}
	...
	// it seems that after some Raspbian update, regular users are sometimes no
	// longer allowed
	//   to use the direct-rendering infrastructure; i.e. the open the devices
	//   inside /dev/dri/ as read-write. flutter-pi must be run as root then.
	// sometimes it works fine without root, sometimes it doesn't.
	if (strncmp(flutterpi.egl.renderer, "llvmpipe", sizeof("llvmpipe") - 1) ==
		0)
	{
		printf("WARNING: Detected llvmpipe (ie. software rendering) as the OpenGL "
			   "ES renderer.\n"
			   "         Check that flutter-pi has permission to use the 3D "
			   "graphics hardware,\n"
			   "         or try running it as root.\n"
			   "         This warning will probably result in a \"failed to set "
			   "mode\" error\n"
	...
	{
		fprintf(stderr,
				"[flutter-pi] Could not clear OpenGL ES context. eglMakeCurrent: "
				"0x%08X\n",
				egl_error);
		return EIO;
	}
	...
	ok = cqueue_init(&flutterpi.frame_queue, sizeof(struct frame),
...
static int init_application(void)
{
	...
	if (libflutter_engine_handle == NULL)
	{
		...
		{
			libflutter_engine_handle =
				dlopen("libflutter_engine.so.release", RTLD_LOCAL | RTLD_NOW);
			if (libflutter_engine_handle == NULL)
			{
				LOG_FLUTTERPI_ERROR("[flutter-pi] Warning: Could not load "
									"libflutter_engine.so.release: %s. Trying to open "
		...
		{
			libflutter_engine_handle =
				dlopen("libflutter_engine.so.profile", RTLD_LOCAL | RTLD_NOW);
			if (libflutter_engine_handle == NULL)
			{
				LOG_FLUTTERPI_ERROR("[flutter-pi] Warning: Could not load "
									"libflutter_engine.so.profile: %s. Trying to open "
									"libflutter_engine.so...\n",
									dlerror());
			}
		}
		...
		{
			libflutter_engine_handle =
				dlopen("libflutter_engine.so.debug", RTLD_LOCAL | RTLD_NOW);
			if (libflutter_engine_handle == NULL)
			{
				LOG_FLUTTERPI_ERROR(
					"[flutter-pi] Warning: Could not load libflutter_engine.so.debug: "
					"%s. Trying to open libflutter_engine.so...\n",
					dlerror());
			}
		}
	}
	...
	{
		libflutter_engine_handle =
			dlopen("libflutter_engine.so", RTLD_LOCAL | RTLD_NOW);
		if (libflutter_engine_handle == NULL)
		{
			LOG_FLUTTERPI_ERROR("Could not load libflutter_engine.so. dlopen: %s",
								dlerror());
			fprintf(stderr,
...
#define LOAD_LIBFLUTTER_ENGINE_PROC(name)                                           \
	...
	{                                                                               \
		...
		{                                                                           \
			perror(                                                                 \
				"[flutter-pi] Could not resolve libflutter_engine procedure " #name \
				". dlsym");                                                         \
			return EINVAL;                                                          \
		}                                                                           \
	} while (false)
...
#undef LOAD_LIBFLUTTER_ENGINE_PROC
				...
				.platform_task_runner =
					&(FlutterTaskRunnerDescription){
						...
						.runs_task_on_current_thread_callback =
							runs_platform_tasks_on_current_thread,
	...
	bool engine_is_aot =
		libflutter_engine->FlutterEngineRunsAOTCompiledDartCode();
	...
	else if ((engine_is_aot == true) &&
			 (flutterpi.flutter.runtime_mode != kProfile))
	{
		fprintf(stderr,
	...
	else if ((engine_is_aot == false) &&
			 (flutterpi.flutter.runtime_mode != kDebug))
	{
		fprintf(stderr,
				"[flutter-pi] The flutter engine was built for debug mode,\n"
				"             but flutter-pi was started up in release mode.\n"
		...
		(flutterpi.flutter.runtime_mode == kProfile))
	{
		...
		engine_result =
			libflutter_engine->FlutterEngineCreateAOTData(&aot_source, &aot_data);
		if (engine_result != kSuccess)
		{
			fprintf(stderr,
					"[flutter-pi] Could not load AOT data. "
					"FlutterEngineCreateAOTData: %s\n",
	...
	// spin up the engine
	engine_result = libflutter_engine->FlutterEngineInitialize(
		FLUTTER_ENGINE_VERSION, &renderer_config, &project_args, NULL,
		&flutterpi.flutter.engine);
	if (engine_result != kSuccess)
	{
		LOG_FLUTTERPI_ERROR("Could not initialize the flutter engine. "
							"FlutterEngineInitialize: %s\n",
							FLUTTER_RESULT_TO_STRING(engine_result));
		return EINVAL;
	}

	engine_result =
		libflutter_engine->FlutterEngineRunInitialized(flutterpi.flutter.engine);
	if (engine_result != kSuccess)
	{
		LOG_FLUTTERPI_ERROR(
			"Could not run the flutter engine. FlutterEngineRunInitialized: %s\n",
			FLUTTER_RESULT_TO_STRING(engine_result));
		return EINVAL;
	}
	...
	engine_result = libflutter_engine->FlutterEngineNotifyDisplayUpdate(
		flutterpi.flutter.engine, kFlutterEngineDisplaysUpdateTypeStartup,
	...
	if (engine_result != kSuccess)
	{
		LOG_FLUTTERPI_ERROR("Could not send display update to flutter engine. "
							"FlutterEngineNotifyDisplayUpdate: %s\n",
							FLUTTER_RESULT_TO_STRING(engine_result));
		return EINVAL;
	}
	...
	if (engine_result != kSuccess)
	{
		LOG_FLUTTERPI_ERROR("Could not send window metrics to flutter engine. "
							"FlutterEngineSendWindowMetricsEvent: %s\n",
							FLUTTER_RESULT_TO_STRING(engine_result));
		return EINVAL;
	}
...
int flutterpi_schedule_exit(void)
{
	...
	if (ok < 0)
	{
		fprintf(
			stderr,
			"[flutter-pi] Could not schedule application exit. sd_event_exit: %s\n",
			strerror(-ok));
		if (pthread_self() != flutterpi.event_loop_thread)
		{
			pthread_mutex_unlock(&flutterpi.event_loop_mutex);
 ...
 * USER INPUT *
 **************/
static void on_flutter_pointer_event(void *userdata,
									 ...
									 size_t n_events)
{
	...
	engine_result =
		flutterpi->flutter.libflutter_engine.FlutterEngineSendPointerEvent(
			flutterpi->flutter.engine, events, n_events);

	if (engine_result != kSuccess)
	{
		LOG_FLUTTERPI_ERROR("Error sending touchscreen / mouse events to flutter. "
...
static void on_xkb_keysym(void *userdata, xkb_keysym_t keysym)
{
	...
	{
		LOG_FLUTTERPI_ERROR(
			"Error handling keyboard event. textin_on_xkb_keysym: %s\n",
			strerror(ok));
		// flutterpi_schedule_exit(flutterpi);
	}
#endif
...
static void on_gtk_keyevent(void *userdata, uint32_t unicode_scalar_values,
							uint32_t key_code, uint32_t scan_code,
							uint32_t modifiers, bool is_down)
{
	FlutterEngineResult engine_result;
	struct flutterpi *flutterpi;
	int ok;
	...
	ok = rawkb_send_gtk_keyevent(unicode_scalar_values, key_code, scan_code,
								 modifiers, is_down);
	if (ok != 0)
	{
		LOG_FLUTTERPI_ERROR(
...
static void on_set_cursor_enabled(void *userdata, bool enabled)
{
	...
	ok = compositor_apply_cursor_state(enabled, flutterpi->view.rotation,
									   flutterpi->display.pixel_ratio);
	if (ok != 0)
	{
		LOG_FLUTTERPI_ERROR("Error enabling / disabling mouse cursor. "
							"compositor_apply_cursor_state: %s\n",
							strerror(ok));
	}
}
...
static int on_user_input_fd_ready(sd_event_source *s, int fd, uint32_t revents,
								  void *userdata)
{
...
static int init_user_input(void)
{
	...
	input = user_input_new(&user_input_interface, &flutterpi,
						   &flutterpi.view.display_to_view_transform,
						   &flutterpi.view.view_to_display_transform,
						   flutterpi.display.width, flutterpi.display.height);
	if (input == NULL)
	{
		LOG_FLUTTERPI_ERROR("Couldn't initialize user input. flutter-pi will run "
							"without user input.\n");
	}
	else
	{
		ok = sd_event_add_io(
			flutterpi.event_loop, &event_source, user_input_get_fd(input),
			EPOLLIN | EPOLLRDHUP | EPOLLPRI, on_user_input_fd_ready, input);
		if (ok < 0)
		{
			LOG_FLUTTERPI_ERROR("Couldn't listen for user input. flutter-pi will run "
								"without user input. sd_event_add_io: %s\n",
								strerror(-ok));
...
#define PATH_EXISTS(path) (access((path), R_OK) == 0)
	...
	asprintf(&kernel_blob_path, "%s/kernel_blob.bin",
			 flutterpi.flutter.asset_bundle_path);
	...
	if (flutterpi.flutter.runtime_mode == kDebug)
	{
		...
		{
			fprintf(stderr,
					"[flutter-pi] Could not find \"kernel.blob\" file inside \"%s\", "
					"which is required for debug mode.\n",
					flutterpi.flutter.asset_bundle_path);
			return false;
		}
	}
	else if (flutterpi.flutter.runtime_mode == kProfile)
	{
		...
		{
			fprintf(stderr,
					"[flutter-pi] Could not find \"app.so\" file inside \"%s\", "
					"which is required for release and profile mode.\n",
					flutterpi.flutter.asset_bundle_path);
			return false;
		}
	}
	...
	{
		fprintf(stderr, "[flutter-pi] Could not find \"icudtl.dat\" file inside "
						"\"/usr/lib/\".\n");
		return false;
	}
...
static bool parse_cmd_args(int argc, char **argv)
{
	...
	while (!finished_parsing_options)
	{
		...
		case 'o':
			...
			else
			{
				fprintf(stderr,
						...
						"Valid values are \"portrait_up\", \"landscape_left\", "
						"\"portrait_down\", \"landscape_right\".\n"
						"%s",
						usage);
				return false;
		...
		case 'r':
			...
			if ((errno != 0) || ((rotation != 0) && (rotation != 90) &&
