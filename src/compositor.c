...
struct compositor compositor = { .drmdev = NULL,
...
static int rendertarget_gbm_present(struct rendertarget *target, struct kms_req_builder *builder) {
 ...
 */
static int rendertarget_gbm_new(struct rendertarget **out, struct compositor *compositor) {
    struct rendertarget *target;
    ...
    *target = (struct rendertarget){
        ...
        .gbm = { .gbm_surface = flutterpi.gbm.surface, .current_front_bo = NULL },
        .gl_fbo_id = 0,
 ...
 * @param[in] userdata A pointer to the flutterpi compositor.
 */
static bool on_create_backing_store(const FlutterBackingStoreConfig *config, FlutterBackingStore *backing_store_out, void *userdata) {
    struct flutterpi_backing_store *store;
    struct rendertarget *target;
    struct compositor *compositor;
    int ok;
    ...
    if (target == NULL) {
        ...
        ok = rendertarget_gbm_new(&target, compositor);
...
extern void on_pageflip_event(int fd, unsigned int frame, unsigned int sec, unsigned int usec, void *userdata);
...
/// PRESENT FUNCS
static bool on_present_layers(const FlutterLayer **layers, size_t layers_count, void *userdata) {
    ...
    for (int i = 0; i < layers_count; i++) {
        if (layers[i]->type == kFlutterLayerContentTypeBackingStore) {
            LOG_DEBUG(
                "  backing store (offset: %f, %f. size: %f, %f)\n",
                layers[i]->offset.x,
                layers[i]->offset.y,
                layers[i]->size.width,
            ...
            for (size_t j = 0; j < layers[i]->platform_view->mutations_count; j++) {
                ...
                switch (mut->type) {
                    case kFlutterPlatformViewMutationTypeOpacity: LOG_DEBUG("    opacity %f\n", mut->opacity); break;
                    case kFlutterPlatformViewMutationTypeClipRect:
                        LOG_DEBUG(
                            "    clip rect (ltrb: %f, %f, %f, %f)\n",
                            mut->clip_rect.left,
                            mut->clip_rect.top,
                            mut->clip_rect.right,
    ...
    if (stored_display != flutterpi.egl.display || stored_read_surface != flutterpi.egl.surface ||
        stored_write_surface != flutterpi.egl.surface || stored_context == EGL_NO_CONTEXT) {
    ...
    for (int i = 0; i < layers_count; i++) {
        if (layers[i]->type == kFlutterLayerContentTypeBackingStore) {
            ...
            if ((cb_data != NULL) && (cb_data->present != NULL)) {
                ...
                ok = cb_data->present(cb_data->view_id, builder, &params, cb_data->userdata);
                if (ok != 0) {
...
const FlutterCompositor flutter_compositor = { .struct_size = sizeof(FlutterCompositor),
